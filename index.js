...
app.get(["/", "/personaltraining.html"], function (req, res) {
  ...
  if (req.session.user) {
    if (req.session.user == "-10") {
      ...
      var getTrainerData =
        "SELECT * FROM trainer WHERE id = '" + req.session.user + "' ";
      ...
      var getUser =
        "SELECT * FROM members WHERE id ='" + req.session.user + "'";
      con.query(getUser, function (error, result) {
        if (error) throw error;
        if (result.length) {
          var getUser =
            "SELECT * FROM register WHERE id ='" + req.session.user + "'";
          con.query(getUser, function (error, result) {
            if (error) throw error;
            if (result.length) {
              // go to member page
              ...
              res.render(__dirname + "/member.html", {
                ...
                data: 1,
                type: result[0].personaltraining,
...
app.post(["/registration.html"], function (req, res) {
  ...
  var register =
    "INSERT INTO `register` (`registerid`,`id`, `firstname`, `lastname`, `address1`, `address2`, `city`, `state`, `currentweight`, `goalweight`, `age`, `height`, `healthcondition`, `personaltraining`) VALUES (NULL, '" +
    req.session.user +
    "', '" +
    k.a1 +
    "', '" +
  ...
  con.query(register, function (error, result) {
    ...
    if (result.affectedRows == 1) {
      ...
      var getUser =
        "SELECT * FROM members,register WHERE register.id ='" +
        req.session.user +
        "'";
      con.query(getUser, function (error, result) {
        if (error) throw error;
...
app.get("/verify.html", function (req, res) {
  ...
  verifyuser =
    "UPDATE members SET verified = 1 WHERE id = '" +
    checkid +
    "' AND verifycode = '" +
    checkcode +
    "'";
...
app.post("/signup.html", function (req, res) {
  ...
  con.query(checkAccount, function (error, result) {
    ...
    if (result.length) {
      ...
      res.send(
        "Email ID already registered with us. <br> " +
      ...
      var sql =
        "Insert into members (email,password,phone,verifycode) values ('" +
        email +
        "','" +
        password +
        "','" +
      ...
      con.query(sql, function (error, result) {
        ...
        var message =
          ...
          "<a href='127.0.0.1/verify.html?verifycode=" +
          verifycode +
          "&userid=" +
          result.insertId +
          "'> Click Here to Verify you Email </a>";
        ...
        res.send(
          "Gym Member Register Successful ! <br> Email Verification Credentials sent to your Email "
        );
...
app.post("/personaltraining.html", function (req, res) {
  ...
  var updateUser =
    "UPDATE register SET personaltraining = 'Personal', " +
    "shoulder = '" +
    k.b1 +
    "' , " +
    "chest = '" +
...
app.post("/addTrainer.html", function (req, res) {
  ...
  con.query(checkAccount, function (error, result) {
    ...
    if (result.length) {
      ...
      var sql =
        "Insert into trainer (name,email,password,phone) values " +
        "('" +
        name +
        "','" +
        email +
...
app.get(["/viewMember.html"], function (req, res) {
  var checkAccount =
    "SELECT members.id , assignedtrainer.memberid , members.email , members.password , members.phone " +
...
app.post(["/assignTrainer.html"], function (req, res) {
  ...
  con.query(checkAccount, function (error, result) {
    ...
    res.render(__dirname + "/assignTrainer.html", {
      data: result,
...
app.post(["/saveAssignedTrainer.html"], function (req, res) {
  ...
  var insertEntry =
    "INSERT INTO assignedtrainer(memberid,trainerid) VALUES('" +
    user1 +
    "' , '" +
    user2 +
    "') ";
...
app.get("/memberChat.html", function (req, res) {
  if (req.session.user && req.session.user != -1) {
    var checkAssignedTrainer =
      "SELECT * FROM assignedtrainer,trainer WHERE memberid = '" +
      req.session.user +
      "' AND trainerid = trainer.id ";
    req.session.user = req.session.user;
    con.query(checkAssignedTrainer, function (error, result) {
      ...
      if (result.length) {
        ...
        var getChat =
          "SELECT * FROM chat WHERE memberid = '" + req.session.user + "' ";

        con.query(getChat, function (error, prevChat) {
          if (error) throw error;

          console.log(prevChat);
          res.render(__dirname + "/memberChat.html", {
            trainer: 1,
            data: result,
            chat: prevChat,
          });
    ...
    con.query(checkAssignedTrainer, function (error, result) {
      ...
      if (result.length) {
        ...
        if (req.body.message) {
          ...
          var saveMessage =
            "Insert into chat(memberid,trainerid,sender,message)" +
            "values('" +
            memid +
            "','" +
            trainid +
          ...
          con.query(saveMessage, function (error, msgSaved) {
            ...
            var getChat =
              "SELECT * FROM chat WHERE memberid = '" + memid + "' ";
